...
- Se elimina el parámetro 'progress' de las llamadas al núcleo para ffpfsc/ffpfs (no soportado en v0.0.9).
- **Corrección en exFAT**: se pasa un objeto StdoutProgress a write_exfat_image.
- **Eliminada la llamada a super().step() en StdoutProgress para suprimir la barra de progreso en consola.**
- **El padre calcula la velocidad y ETA a partir del progreso y el tiempo transcurrido.**
- **Carga de payload_sender usando importlib.util para que funcione en el ejecutable compilado.**
- **Añadida visualización del icono del juego (sce_sys/icon0.png) al cargar una carpeta.**
- **Añadida generación de ampr_emu.index:**
   * Automática (con ensure_ampr_index) para ffpfsc, ffpfs y exfat si el checkbox está marcado.
   * Manual (con build_ampr_index.py) mediante botón específico, sin depender de fakelib.
- **Eliminada la sección "Carpeta temporal" por no ser utilizada en los flujos actuales.**
"""
...
# ---- IMPORTAR build_ampr_index (para generación manual sin dependencia de fakelib) ----
try:
    import build_ampr_index as ampr_builder
    _AMPR_BUILDER_AVAILABLE = True
except ImportError:
    ampr_builder = None
    _AMPR_BUILDER_AVAILABLE = False

# ---- AÑADIR RUTA DE PYINSTALLER PARA MÓDULOS EMPAQUETADOS ----
if getattr(sys, 'frozen', False):
    sys.path.insert(0, sys._MEIPASS)

# ---- IMPORTAR PAYLOAD_SENDER (con fallback para ejecutable compilado) ----
if getattr(sys, 'frozen', False):
    # En ejecutable empaquetado, cargar desde el archivo físico extraído
    try:
        import importlib.util
        payload_path = os.path.join(sys._MEIPASS, "payload_sender.py")
        if os.path.isfile(payload_path):
            spec = importlib.util.spec_from_file_location("payload_sender", payload_path)
            if spec and spec.loader:
                payload_sender = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(payload_sender)
            else:
                payload_sender = None
        else:
            payload_sender = None
    except Exception as e:
        print(f"Error cargando payload_sender desde el ejecutable: {e}", file=sys.stderr)
        payload_sender = None
else:
    # En modo desarrollo, importar normalmente
    try:
        import payload_sender
    except ImportError as e:
        print(f"Error importando payload_sender: {e}", file=sys.stderr)
        payload_sender = None

# ============================================================================
# 0. PROGRESS PARA SALIDA ESTÁNDAR (MODO WORKER)
# ============================================================================
class StdoutProgress(Progress):
    """
    Implementación de Progress que escribe líneas estructuradas en stdout.
    No llama a super().step() para evitar la salida de la barra de progreso por defecto.
    """
    ...
    def step(self, phase: str, done: int, total: int, bytes_processed: int = 0) -> None:
        # NO llamamos a super().step() para evitar la salida de la barra en consola
        pct = (done / total) * 100.0 if total > 0 else 0.0
        sys.stdout.write(f"PROGRESS|{pct:.2f}|{phase}\n")
        sys.stdout.flush()

    def status(self, message: str) -> None:
        # NO llamamos a super().status()
        sys.stdout.write(f"STATUS|{message}\n")
        sys.stdout.flush()


# ============================================================================
# 0b. FUNCIÓN DE CONVERSIÓN PARA MODO WORKER
# ============================================================================
def run_conversion_task(
    ...
    Para exFAT se pasa un StdoutProgress.
    Para los otros formatos no se pasa progress (no soportado).
    """
    is_dir = src_path.is_dir()

    # ===== exFAT (SÍ soporta progress) =====
    if format_type == "exfat":
        ...
        if cluster_size.lower() != "auto":
            try:
                ...
                pass
        progress = StdoutProgress(enabled=not silent)
        write_exfat_image(
    ...
    # ===== ffpfsc (comprimido) - NO soporta progress =====
    ...
    # ===== ffpfs (sin comprimir) - NO soporta progress =====
...
# ============================================================================
# 4. PESTAÑA DE CONVERSIÓN (modificada para calcular velocidad/ETA localmente y mostrar icono)
# ============================================================================
class ConversionTab(QWidget):
    ...
    def __init__(self):
        ...
        self._last_bytes = 0  # Para estimar velocidad
        self._total_bytes = 0  # Estimado del tamaño total (se obtiene de la primera actualización)
        self._timer = QTimer()
    ...
    def setup_ui(self):
        ...
        # Fila con el campo de entrada y el icono
        ...
        source_row.addWidget(self.icon_label)
        ...
        # --- SECCIÓN: Generación de ampr_emu.index ---
        ampr_layout = QHBoxLayout()
        self.generate_ampr_btn = QPushButton("📄 Generar ampr_emu.index")
        self.generate_ampr_btn.setEnabled(False)
        self.generate_ampr_btn.clicked.connect(self._generate_ampr_index)
        self.generate_ampr_btn.setToolTip("Genera el índice ampr_emu.index usando build_ampr_index.py (sin depender de fakelib)")
        ampr_layout.addWidget(self.generate_ampr_btn)

        self.auto_ampr_check = QCheckBox("Generar índice automáticamente al convertir")
        self.auto_ampr_check.setChecked(True)
        self.auto_ampr_check.setToolTip("Si está activado, se generará ampr_emu.index automáticamente antes de cada conversión (para ffpfsc, ffpfs y exfat)")
        ampr_layout.addWidget(self.auto_ampr_check)
        ampr_layout.addStretch()
        source_layout.addLayout(ampr_layout)
        # --- FIN SECCIÓN ---
        ...
        # --- SECCIÓN ELIMINADA: Carpeta temporal ---
    ...
    def browse_file(self):
        ...
        if path:
            ...
            self._run_preflight()
            self._load_icon()
            self._update_ampr_button_state()
    ...
    def _on_source_changed(self, text):
        """Se dispara cuando el usuario edita manualmente la ruta."""
        self._load_icon()
        self._update_ampr_button_state()

    def _load_icon(self):
        """Busca sce_sys/icon0.png en la ruta seleccionada (si es carpeta) y la muestra."""
        ...
        if not src_path.is_dir():
            self.icon_label.setVisible(False)
            return
        icon_path = src_path / "sce_sys" / "icon0.png"
    ...
    def _update_ampr_button_state(self):
        """Habilita/deshabilita el botón de generar ampr_emu.index según si hay una carpeta cargada."""
        path_str = self.source_edit.text().strip()
        if path_str and Path(path_str).is_dir():
            self.generate_ampr_btn.setEnabled(True)
        else:
            self.generate_ampr_btn.setEnabled(False)

    def _generate_ampr_index(self):
        """Genera el índice ampr_emu.index usando build_ampr_index (siempre, sin depender de fakelib)."""
        path_str = self.source_edit.text().strip()
        if not path_str:
            QMessageBox.warning(self, "Error", "Selecciona una carpeta primero.")
            return
        src_path = Path(path_str)
        if not src_path.is_dir():
            QMessageBox.warning(self, "Error", "La ruta debe ser una carpeta.")
            return

        if not _AMPR_BUILDER_AVAILABLE or ampr_builder is None:
            QMessageBox.critical(self, "Error",
                "No se encontró el módulo build_ampr_index.py.\n"
                "Asegúrate de que el archivo esté en el directorio de la aplicación.\n"
                "Si estás usando el ejecutable compilado, incluye el script como recurso.")
            return

        # Verificar si ya existe
        index_path = src_path / "ampr_emu.index"
        if index_path.exists():
            reply = QMessageBox.question(self, "Índice existente",
                                         "Ya existe un archivo ampr_emu.index. ¿Deseas sobrescribirlo?",
                                         QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply != QMessageBox.StandardButton.Yes:
                return

        try:
            # Usar la función build_index_local del script
            # Devuelve 0 si éxito, distinto de 0 si error.
            ret = ampr_builder.build_index_local(src_path, index_path, allow_case_collisions=False)
            if ret == 0:
                QMessageBox.information(self, "Éxito", f"Índice generado correctamente en:\n{index_path}")
                # Refrescar el icono (por si cambia algo)
                self._load_icon()
            else:
                QMessageBox.critical(self, "Error", f"Error al generar el índice. Código de retorno: {ret}")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al generar el índice:\n{e}")
    ...
    def run_conversion(self):
        ...
        # --- GENERACIÓN AUTOMÁTICA DE ampr_emu.index (ahora también para exFAT) ---
        if self._current_format in ("ffpfsc", "ffpfs", "exfat") and self.auto_ampr_check.isChecked():
            try:
                # Usamos ensure_ampr_index (que solo genera si existe fakelib/libSceAmpr.sprx)
                result = ensure_ampr_index(src_path, enabled=True)
                if result:
                    self._append_log(f"✅ ampr_emu.index generado automáticamente en: {result}")
                else:
                    # No se generó porque no hay marcador, no es un error
                    pass
            except Exception as e:
                # Mostrar advertencia pero continuar (no es crítico)
                self._append_log(f"⚠️ Advertencia al generar ampr_emu.index automáticamente: {e}")
    ...
    def _update_progress(self, percent, phase):
        ...
        # Calcular velocidad y ETA localmente
        elapsed = time.time() - self._start_time
        if elapsed > 1.0 and percent > 0:
            # Estimar bytes procesados a partir del porcentaje (asumiendo que el total es 100%)
            # No tenemos bytes totales, pero podemos estimar velocidad a partir del cambio de porcentaje
            # Usamos la última actualización para calcular la velocidad en %/s y luego estimamos MB/s
            # Mejor: usar un valor fijo para demostración, pero lo correcto es usar bytes_processed
            # Como no tenemos bytes_processed, usamos una estimación basada en el tamaño total conocido (de preflight)
            # Obtenemos el tamaño total de la preflight
            src = self.source_edit.text().strip()
            if src:
                try:
                    src_path = Path(src)
                    ...
                    # Velocidad = (percent/100 * total_bytes) / elapsed
                    speed = (percent / 100.0 * total_bytes) / elapsed
                    eta = ((100 - percent) / 100.0 * total_bytes) / speed if speed > 0 else 0
    ...
    def _update_stats(self, stats):
        # Esta función ahora solo se usa para otros formatos (ffpfsc, ffpfs) cuando se emite stats_signal
        # Para exFAT, ya calculamos en _update_progress
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MkPFS - Convertidor + Payload Sender (ModGames44), VER 2.0")
        ...
        title_label = QLabel("MKPFS EXFAT PS🎮")
        font = QFont()
